...
export const SMSConfig = () => {
  ...
  const fetchData = async (value = null) => {
    ...
    for (let i = 0; i < countObjHoliday.length; i++) {
      ...
      countObjHoliday[i].srno = page_limit * current_page - (page_limit - i - 1); // srno added to response thank you jigi
  ...
  const hanldeSearch_holidayList = (value, event) => {
    ...
    const fetchDataOnSearch = async (value) => {
      ...
      for (let i = 0; i < countObjHolidaySr.length; i++) {
        ...
        let current_page = tableParams.pagination.current;
        let page_limit = tableParams.pagination.pageSize;
        countObjHolidaySr[i].srno = page_limit * current_page - (page_limit - i - 1); // srno added to response thank you jigi
      ...
      <Row gutter={16}>
        <Col xs={24} sm={24} md={24} lg={24} xl={24}>
          ...
          <Flex alignItems="center" className=" mb-3 __card" justifyContent="space-between" mobileFlex={false} >	
            ...
            {/* search bar for holiday list table */}
